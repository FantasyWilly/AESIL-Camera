...
from camera_ktg_pkg.lib.ktg_publisher import GCUPublisher
from camera_ktg_pkg.lib.ktg_camera_decoder import ReceiveMsg
...
class CommunicationController:
    ...
    def loop_send_command(self, loop_cmd_bytes):
        ...
        for packet in self._extract_packets(response):
            self._decode_and_publish(packet)

        return response

    # ---------- (_extract_packets) 從回傳資料取出完整封包 ----------
    def _extract_packets(
        self,
        data: bytes,
        packet_size: int = 32,
        header: bytes = b'\x4B\x4B',
    ) -> List[bytes]:
        packets = []
        ...
        while len(self.recv_buffer) >= packet_size:
            start_idx = self.recv_buffer.find(header)
            if start_idx == -1:
                self.recv_buffer.clear()
                break

            if start_idx > 0:
                del self.recv_buffer[:start_idx]

            if len(self.recv_buffer) < packet_size:
                break

            packet = bytes(self.recv_buffer[:packet_size])
            del self.recv_buffer[:packet_size]
            packets.append(packet)

        return packets

    # ---------- (_decode_and_publish) 解碼封包並發布 ROS2 訊息 ----------
    def _decode_and_publish(self, packet: bytes) -> bool:
        if not self.gimbal_msg.parse(packet, len(packet), self.gimbal_msg):
            print("無法解析資料")
            return False

        self.ros2_publisher_node.publish_camera_data(
            self.gimbal_msg.rollAngle,
            self.gimbal_msg.pitchAngle,
            self.gimbal_msg.yawAngle,
            self.gimbal_msg.eoZoom,
            self.gimbal_msg.targetDist,
        )
        return True
